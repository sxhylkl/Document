...
# 进程文件,刚安装的时候没有，在启动nginx时自动生成的,里面存放的是当前nginx进程的ID号
pid /var/run/nginx.pid;
# nginx的结束重启命令,其中26000是nginx的主进程号：
kill -QUIT 26000

# 错误日志位置和级别，debug、info、notice、warn、error、crit,默认为crit
# 说明：crit 记录的日志最少，而debug记录的日志最多
